...
| BMAD Method | Multi-agent orchestration (roles: PM, Architect, Dev, QA) | S1-S6 maps to BMAD story format |
...
[BMAD Method](https://github.com/bmad-method/BMAD-METHOD) uses role-based agents (PM, Architect, Developer, QA) that process features through a structured handoff pipeline. BMAD handles *who does what*; spec-first handles *what is complete enough to implement*. They are complementary — not competing.
...
**Workflow**: Write the spec using spec-first → feed to BMAD's PM role for story decomposition → Architect for technical breakdown.
